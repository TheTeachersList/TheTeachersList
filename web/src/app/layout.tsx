    ...
    <header className="bg-board border-b-4 border-gold">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
        <Link href="/" className="flex items-baseline gap-2 shrink-0">
          ...
          <span className="hidden sm:inline font-hand text-gold text-base -rotate-2">
            gifts, sorted.
          </span>
        </Link>
        <nav className="flex items-center gap-4 sm:gap-6 text-sm font-medium text-paper-dark">
          <Link href="/" className="hover:text-gold">
            Home
          </Link>
          <Link href="/about" className="hover:text-gold hidden sm:inline">
            About
          </Link>
          <Link href="/contact" className="hover:text-gold hidden sm:inline">
            Contact
          </Link>
          <Link
            href="/add-profile"
            className="bg-brick hover:bg-brick-dark text-white rounded-[4px] px-3 py-2 text-sm font-semibold"
          >
            Add my profile
          </Link>
        </nav>
  ...
  return (
    <footer className="border-t hairline mt-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-ink-soft">
        <p>&copy; {new Date().getFullYear()} The Teacher&apos;s List</p>
